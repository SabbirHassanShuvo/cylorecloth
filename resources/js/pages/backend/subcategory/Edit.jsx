...
const Edit = () => {
    const { subcategory, categories, error } = usePage().props; // Inertia.js
    ...
    const onSubmit = (data) => {
        setIsUpdating(true);
        router.post(`/subcategory/update/${subcategory.id}`, data, {
            onSuccess: () => {
                toast.success("Category Updated Successfully!", { autoClose: 2000 });

                setTimeout(() => {
                    router.visit("/subcategory"); // ✅ Inertia Redirect
                }, 2000);
                ...
                <div className="flex flex-col md:flex-row">
                    ...
                    <div className="w-full p-4 pt-0">
                        <header className="flex items-center justify-between px-3 py-2 bg-white shadow-lg rounded-lg">
                            <h1 className="text-xl font-semibold text-gray-700">Edit Sub Category</h1>
                        ...
                        <div className="flex justify-center items-center min-h-screen bg-gray-100 p-4">
                            <div className="bg-white p-6 md:p-8 rounded-2xl shadow-2xl w-full max-w-md">
                                <h2 className="text-xl font-semibold mb-4 text-gray-700 text-center">Edit Sub Category</h2>
                                <form onSubmit={handleSubmit(onSubmit)} className="grid gap-4">
                                    ...
                                    <div>
                                        ...
                                        <select
                                            {...register("status")}
                                            defaultValue={String(subcategory?.status ?? "1")} // ✅ Ensure string format for safety
                                            className="w-full p-3 border border-gray-300 rounded-lg shadow-md transition-all focus:outline-none focus:ring-2 focus:ring-blue-400 focus:shadow-lg"
                                        >
