...
const SubCategory = ({ filters }) => {
    ...
    const handleDelete = () => {
        ...
        router.get(`/subcategory/delete/${selectedUserId}`, {
            onSuccess: (page) => {
    ...
    const handleSearch = (e) => {
        e.preventDefault();
        get('/subcategory', { preserveState: true, replace: true });
    };
    return (
        <>
            {/* Header Part */}
                    ...
                    <div className="w-full p-4 pt-0">
                        <header className="flex items-center justify-between px-3 py-2 bg-white shadow-lg rounded-lg">
                            ...
                            <div className="flex-1">
                                <h1 className="text-xl font-semibold text-gray-700">Sub Category</h1>
                            </div>
                            ...
                            <div className="flex-3 flex items-center gap-4 justify-end w-full">
                                ...
                                <div className="flex gap-4">
                                    <Link href="/subcategory/create" className="px-4 py-2 bg-blue-600 text-white font-medium rounded-lg shadow-md hover:bg-blue-700">
                                        Create Sub Category
                                    </Link>
                                </div>
                            </div>
                                            ...
                                            <tbody className="divide-y divide-gray-200">
                                                {subcategories.data.length > 0 ? (
                                                    subcategories.data.map((subcategories) => (
                                                        <tr key={subcategories.id} className="hover:bg-gray-100 transition">
                                                            <td className="px-6 py-2.5 text-sm text-gray-800">{subcategories.name}</td>
                                                            <td className="px-6 py-2.5 text-sm text-gray-800">
                                                                {subcategories.category?.name ? subcategories.category.name : 'No Category'}
                                                            </td>
                                                            ...
                                                            <td className="px-6 py-2.5 text-sm text-gray-800">
                                                                <div className="flex justify-end space-x-3">
                                                                    <Link href={`/subcategory/edit/${subcategories.id}`} className="text-yellow-500 hover:text-yellow-700">
                                                                        <FaEdit className="w-4 h-4" />
                                                                    </Link>
                                                                    <button
                                                                        onClick={(e) => {
                                                                            e.preventDefault();
                                                                            openModal(subcategories.id);
                                                                        }}
                                                    ...
                                                    <tr>
                                                        <td colSpan="4" className="px-6 py-6 text-center text-gray-500 bg-gray-100 rounded-lg">
                                                            <div className="flex flex-col items-center justify-center space-y-3">
                                                                ...
                                                                <p className="text-lg font-medium text-gray-700">No Sub Category found</p>
                                            ...
                                            createPortal(
                                                <div className="fixed inset-0 flex items-center justify-center bg-[rgba(221,221,221,0.5)]">
                                                    <div className="bg-white p-5 rounded-lg shadow-lg" onClick={(e) => e.stopPropagation()}>
                                                        ...
                                                        <p className="text-gray-600">Do you really want to delete this Sub Category?</p>
                        ...
                        <nav className="flex flex-wrap justify-center sm:justify-end items-center gap-x-1 mt-4 overflow-x-auto" aria-label="Pagination">
                            <button
                                type="button"
                                onClick={() => handlePagination(subcategories.prev_page_url)}
                                disabled={!subcategories.prev_page_url}
                            ...
                            <button
                                type="button"
                                onClick={() => handlePagination(subcategories.next_page_url)}
                                disabled={!subcategories.next_page_url}
                                className="min-h-9.5 min-w-9.5 py-2 px-2.5 border border-transparent text-gray-800 hover:bg-gray-100 disabled:opacity-50"
                                aria-label="Next"
                            >
                                ▶️
